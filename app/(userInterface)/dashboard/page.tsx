...
function DashboardHeader() {
  ...
  return (
    <div className='w-full h-1/4 flex flex-row justify-around items-center gap-1   rounded-lg '>
      {data.map((item, index) => (
        <div 
          className='flex  items-center gap-6 bg-neutral-800 border  border-neutral-600 p-4  rounded-lg hover:shadow-lg transition-all duration-300 ease-in-out ' 
          key={index}
        ...
        >
          ...
          <div className={`flex-1 p-2 ${item.title === 'Revenue' ? 'bg-green-400' : item.title === 'Total Orders' ? 'bg-sky-400' : item.title === 'Pending Orders' ? 'bg-yellow-400' : item.title === 'Website Visitors' ? 'bg-amber-400' : 'bg-gray-400'} rounded-2xl`}>
            {item.icon}
          </div>
